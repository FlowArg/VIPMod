...
public class ChestPurifierByUpgradeSlot extends AbstractByUpgradeSlot
{
    ChestPurifierByUpgradeSlot(Container container, IInventory inventoryIn, int index, int xPosition, int yPosition)
    {
        ...
        for (Slot slot : this.container.inventorySlots)
        {
            ...
            if(slot instanceof UpgradeSlot)
            {
                if (slot.getHasStack() && slot.getStack().getItem() instanceof UpgradeItem)
                {
                    ...
                    if (upgradeItem.getUpgradeType() == UpgradeType.CHEST_PURIFIER)
                        flag = true;
                }
            }
