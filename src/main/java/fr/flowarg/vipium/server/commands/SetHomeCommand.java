...
public class SetHomeCommand
{
    public static void register(CommandDispatcher<CommandSource> dispatcher)
    {
        dispatcher.register(Commands.literal("sethome").then(Commands.argument("homeName", MessageArgument.message()).executes(context -> {
            ...
            if(Main.serverManager.getHomeCore().addHome(player.getName().getFormattedText(), new Home(MessageArgument.getMessage(context, "homeName").getFormattedText(), player.dimension.getId(), player.serverPosX, player.serverPosY, player.serverPosZ, player.rotationYaw, player.rotationPitch)) != 0)
            {
