...
public final class DataGenerators
{
    ...
    public static void registerDomain(String modid)
    {
        DOMAINS.add(modid);
        if(DOMAINS.size() == 1)
            MinecraftForge.EVENT_BUS.register(new ModelLoadingListener());
    }

    public static void addModelFixer(IModelFixer fixer)
    {
        if (MODEL_FIXERS.contains(fixer))
            throw new IllegalArgumentException("Given argument already exist!");
        else MODEL_FIXERS.add(fixer);
    }

    public static void addModelGenerator(IModelGenerator generator)
    {
        if (MODEL_GENERATORS.contains(generator))
            throw new IllegalArgumentException("Given argument already exist!");
